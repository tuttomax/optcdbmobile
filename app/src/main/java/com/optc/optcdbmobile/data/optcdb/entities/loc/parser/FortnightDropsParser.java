...
public class FortnightDropsParser extends BaseParser<List<FortnightDrops>> {
    @Override
    public List<FortnightDrops> parse(Object jsParsed) {
        List<FortnightDrops> list = new ArrayList<>();

        NativeArray array = (NativeArray) jsParsed;
        for (int index = 0; index < array.size(); index++) {
            FortnightDrops drops = new FortnightDrops();
            NativeObject obj = (NativeObject) array.get(index);


            drops.setName(toType(obj.get("name"), String.class));
            drops.setThumb(toType(obj.get("thumn"), Integer.class));
